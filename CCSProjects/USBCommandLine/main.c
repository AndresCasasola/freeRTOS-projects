...
extern void vUARTTask(void *pvParameters);	// Task in commands.c
...
int main(void){
    ...
    // Tasks Creation
	//BaseType_t t1 = xTaskCreate(Heartbeat, (portCHAR *)"Heartbeat", 512, NULL, tskIDLE_PRIORITY + 1, NULL);
	//if(t1 != pdTRUE)
	//	while(1){}

	BaseType_t t2 = xTaskCreate(vUARTTask, (portCHAR *)"Uart", 512,NULL,tskIDLE_PRIORITY + 1, NULL);
	if(t2 != pdTRUE)
		while(1){}
